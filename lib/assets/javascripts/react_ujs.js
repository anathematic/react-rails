...
(function(document, window) {
  ...
  window.ReactRailsUJS = {
    ...
    mountComponents: function(nodes) {
      if(String(typeof(nodes)) === "object") {
        var nodes = nodes
      } else {
      ...
      for (var i = 0; i < nodes.length; ++i) {
        ...
        var constructor = window.Aeon.Components[className] || eval.call(window, className);
        var propsJson = node.getAttribute(window.ReactRailsUJS.PROPS_ATTR);
        var props = (propsJson && JSON.parse(propsJson)) || {};
        props.innerHTML = node.innerHTML;

        React.render(React.createElement(constructor, props), node, function() {
          $(node).css({display: ""});
          children = node.querySelectorAll('[' + window.ReactRailsUJS.CLASS_NAME_ATTR + ']');

          if(children.length > 0) {
            window.ReactRailsUJS.mountComponents(children);
          }
        });
      }
    },
